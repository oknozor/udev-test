...
fn main() {
    let gpus = Gpus::load();
    println!("{:?}", gpus);
    let switchable = gpus.is_switchable();
    assert!(!switchable);
    let default = gpus.get_default();
    assert!(default.is_some());
    let non_default = gpus.non_default();
    assert!(non_default.is_none());
    println!("{:?}", gpus.default.unwrap().launch_options());
}
